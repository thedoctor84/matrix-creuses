...
map<pair<int, int>, int> ADDITION(map<pair<int, int>, int> A, int Lig_A, int Col_A, map<pair<int, int>, int> B, int Lig_B, int Col_B)
{
	...
	while (it1 != A.end() || it2 != B.end())
	{
		if(it1->first.first == it2->first.first)					// i de A == i de B
		{
			...
			{
				C.insert(pair<pair<int, int>, int> (make_pair(it1->first.first, it1->first.second), (it1->second + it2->second)));
				it1++;
				it2++;
			}
			else													// j de A != j de B
	...
	for(int i=0; i<Lig_A; i++)
	{
		for(int j=0; j<Col_B; j++)
		{
			...
			for (int k=0; k<Col_A; k++) // Boucle qui fait l'opération
			{
				...
				while (it1 != A.end() && it1->first.second < k || it1->first.second == k && (it1->first.first != i)) it1++;
				while (it2 != B.end() && it2->first.first < k || it2->first.first == k && (it1->first.second != j)) it2++;
			}
			C.insert(pair<pair<int, int>, int> (make_pair(i, j), temp));
		}
	}
}